  ...
  return (
    <PageShell>
      <div className="mb-8 hidden flex-col justify-between gap-4 lg:flex lg:flex-row lg:items-center">
        <div>
          <p className="text-sm font-bold uppercase tracking-widest text-[#0EA5E9]">Panel colaborador</p>
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="card p-6">
          <div className="mb-5 flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-[#1E40AF]">Resumen</p>
              <h2 className="text-xl font-black text-[#14264F]">Próximos días asignados</h2>
            </div>
            <Link href="/collaborator/itineraries" className="text-sm font-black text-[#0EA5E9] hover:text-[#1E40AF]">Ver todo</Link>
          </div>
          <div className="space-y-3">
            {recentDays.length ? recentDays.map((day) => (
              <div key={day.id} className="rounded-3xl border border-slate-200 bg-white p-4">
                <p className="text-xs font-black uppercase tracking-widest text-[#0EA5E9]">Día {day.day_number}</p>
                <h3 className="mt-1 font-black text-[#14264F]">{day.title || `Día ${day.day_number}`}</h3>
                {day.route ? <p className="mt-1 text-sm font-semibold text-slate-500">Ruta: {day.route}</p> : null}
              </div>
            )) : (
              <p className="rounded-3xl border border-dashed border-slate-200 bg-white p-6 text-sm font-semibold text-slate-500">Aún no tienes días asignados.</p>
            )}
          </div>
        </div>
        ...
        <div className="grid gap-4">
          <Link href="/collaborator/itineraries" className="card block p-6 hover:ring-2 hover:ring-emerald-500/30">
            <p className="text-sm font-bold uppercase tracking-widest text-[#0EA5E9]">Operación</p>
            <h2 className="mt-2 text-xl font-black text-[#14264F]">Ver mis itinerarios</h2>
          ...
          <Link href="/collaborator/chat" className="card block p-6 hover:ring-2 hover:ring-emerald-500/30">
            <p className="text-sm font-bold uppercase tracking-widest text-[#0EA5E9]">Comunicación</p>
            <h2 className="mt-2 text-xl font-black text-[#14264F]">Abrir chat</h2>
          ...
          <Link href="/collaborator/profile" className="card block p-6 hover:ring-2 hover:ring-emerald-500/30">
            <p className="text-sm font-bold uppercase tracking-widest text-[#0EA5E9]">Perfil</p>
            <h2 className="mt-2 text-xl font-black text-[#14264F]">Editar mis datos</h2>
